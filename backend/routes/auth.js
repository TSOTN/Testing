...
router.post('/register', async (req, res) => {
  ...
  const normalizedEmail = email.toLowerCase(); // Normalizar email

  try {
    // 1. Validar si el usuario ya existe
    const userExist = await pool.query('SELECT * FROM users WHERE email = $1 OR username = $2', [normalizedEmail, username]);
